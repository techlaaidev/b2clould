...
function jpTimeZone_(text) {
  const t = String(text || "").trim();
  if (!t) return "";
  if (t.indexOf("午前") !== -1) return "51";
  const h = (t.match(/\d{1,2}/g) || []).map(Number);
  const start = h[0];
  if (start == null) return "";
  if (start < 12) return "51";
  if (start >= 20) return "56";
  if (start === 19) return "57";
  if (start === 18) return "55";
  if (start >= 16) return "54";
  if (start >= 14) return "53";
  if (start >= 12) return "52";
  return "";
}

// Tên hàng 品名: bỏ nhãn thanh toán đầu + phần giá/phép tính cuối.
function jpItemName_(product) {
...
function jpSplitAddress_(addr) {
  const s = String(addr || "").trim();
  return [s.slice(0, 25), s.slice(25, 50), s.slice(50, 75)];
}
...
// Upload file THẲNG từ máy (không qua Drive): dialog đọc Shift-JIS ngay ở
// trình duyệt rồi gửi text về server xử lý.
...
// Luồng mỗi lần gõ IMEI (chính xác tối đa):
//   1. LUÔN đồng bộ gia tăng: hỏi KiotViet các SP vừa thay đổi → vá index.
//   2. Tra index để lấy MÃ SP.
//   3. Gọi LIVE KiotViet cho đúng máy đó → đọc status/branch → quyết định
...
function kvFillNameFromImei_(e, sheet, headers, token, retailer) {
  ...
  if (!r || !r.hit) {
    ...
    sheet.getRange(row, codeCol).clearContent();
    SpreadsheetApp.getActive().toast(
      'Không tìm thấy sản phẩm nào có IMEI chứa "' +
        typed +
...
// đang giữ máy đó (kiểm tra LIVE với KiotViet ngay lúc này — hàng rào chống bán trùng).
//   - product (id, code, name, basePrice)
//   - branchId: chi nhánh đang giữ máy (17397 hoặc 17637)
...
function kvCreateInvoice_(
  ...
  if (delivery) {
    // Mẫu vận đơn đã học (KV_DELIVERY_TEMPLATE) chỉ dùng cho khối lượng/kích
    // thước/type; ĐỐI TÁC giao hàng lấy theo cột "Đơn vị giao hàng" (delivery.partner).
    let template = null;
    try {
      template = JSON.parse(
